		...
		try {
			...
			if (user) {
				...
				if (wishlistDoc.exists()) {
					...
					const updatedWishlist = wishlistData.vehicleID.filter(
						(id) => id !== vehicleIdToRemove
					);

					// Update the Firestore document
					await setDoc(
						wishlistDocRef,
						{ vehicleID: updatedWishlist },
						{ merge: true }
					);
				}
			} else {
							...
							<View>
								...
								<TouchableOpacity style={styles.removeButton}>
									<Text style={{ color: 'blue' }}>
										{vehicle.bookingStatus === 'accepted'
											? 'Busy'
											: 'Available'}
									</Text>
								</TouchableOpacity>
							...
							<View style={[styles.cardItems, { marginBottom: 7 }]}>
								...
								{booking.bookingStatus === 'pending' ? (
									<TouchableOpacity
										style={[
											styles.viewDetialsButton,
											{ backgroundColor: '#9ea192', marginHorizontal: 'auto' }
										]}
										disabled
									>
	...
	return (
		<Tab.Navigator>
			...
			<Tab.Screen name="ProfileTab">
				{() => <ProfileTab user={user} headerTitle={'Profile'} />}
			</Tab.Screen>
		</Tab.Navigator>
	);
};
