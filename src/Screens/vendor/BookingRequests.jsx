...
const BookingRequests = ({ navigation }) => {
			...
			try {
				...
				const fetchedBookings = await Promise.all(
					querySnapshot.docs.map(async (doc) => {
						...
						const avatarUrl =
							avatarSnapshot.docs.length > 0
								? avatarSnapshot.docs[0].data().downloadUrl
								: null;
	...
	const handleAccept = async (bookingId) => {
		try {
			...
			setBookings(
				bookings.map((b) =>
					b.id === bookingId ? { ...b, bookingStatus: 'accepted' } : b
				)
			);
		} catch (error) {
	...
	return (
		<ScrollView contentContainerStyle={styles.container}>
			<Text style={styles.header}>Demandes de réservation</Text>
			{bookings.length > 0 ? (
					...
					return (
						<View key={booking.id} style={styles.CardContainer}>
							...
							<View
								style={[
									styles.cardItems,
									{
										marginBottom: 7,
										justifyContent:
											isAccepted || isRejected ? 'flex-start' : 'space-between',
										gap: isAccepted || isRejected ? 10 : 0
									}
								]}
							>
								<View
								...
								{isRejected && (
									<TouchableOpacity
										style={[
											styles.viewDetialsButton,
											{ backgroundColor: '#fa6055', marginHorizontal: 'auto' }
										]}
										disabled
									>
								...
								{isAccepted && (
									<TouchableOpacity
										style={[
											styles.viewDetialsButton,
											{ backgroundColor: '#50b379', marginHorizontal: 'auto' }
										]}
										disabled
									>
							...
							<Text style={styles.dateFromTo}>
								Request Sent on {formatBookingDate(booking.bookingDate)}
							</Text>
						</View>
					);
				})
